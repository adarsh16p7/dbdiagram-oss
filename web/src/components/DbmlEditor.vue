<template>
  <div ref="root" class="dbml-editor-wrapper">
    <v-ace-editor class="dbml-editor" lang="dbml" :theme="theme" v-model:value="sourceCode" :print-margin="false" :options="options" @init="editorInit" />
  </div>
</template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const editorInit = (editorInstance) => {
      ace = editorInstance;
      editor.setAceEditorInstance(ace); // Update the store with the editor instance
    };
    ...
    const subscription = editor.$subscribe((mutation, state) => {
      if (mutation.storeId === "editor" && mutation.payload) {
        if (mutation.payload.source && mutation.payload.source.markers && mutation.payload.source.markers.selection) {
          const range = editor.source.markers.selection;
          const startToken = range.start;
          const endToken = range.end;

          highlightTokenRange({ row: startToken.line, col: startToken.column }, { row: endToken.line, col: endToken.column });
        }
